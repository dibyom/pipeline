...
func TestResolvedConditionCheck_ConditionToTaskSpec(t *testing.T) {
	...
	want := &v1alpha1.TaskSpec{
		...
		Steps: []corev1.Container{container},
	}

	if d := cmp.Diff(rcc.ConditionToTaskSpec(), want); d != "" {
		t.Errorf("TaskSpec generated from Condition is unexpected: %v", d)
	}
